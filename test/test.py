...
@views.route('/tracker.html', methods=['GET', 'POST'])
def tracker():
    db_file = "db/database.db"
    table_name = "covid_data"
    ...
    if request.method == "POST":
        ...
        df = read_data_from_db(db_file, table_name)
        df.drop(["Lat", "Long"], axis=1, inplace=True)
        search_result = df[df.apply(lambda row: row.astype(str).str.contains(search_value, case=False).any(), axis=1)]
        search_result = search_result.to_html(index=False) if not search_result.empty else "No matches found."
